...
RSpec.describe Plumbing::Observable do
  ...
  it "delivers a pushed event to a registered observer" do
    ...
    broadcaster.announce(ThingHappened.new(id: "1"))

    expect(events).to eq [ThingHappened.new(id: "1")]
  end
  ...
  it "delivers to every registered observer" do
    ...
    expect(a).to eq [ThingHappened.new(id: "1")]
    expect(b).to eq [ThingHappened.new(id: "1")]
  end
  ...
  it "builds and delivers a registered event via notify" do
    ...
    expect(events).to eq [ThingHappened.new(id: "7")]
  end
  ...
  it "is a safe no-op to emit when nothing is observing" do
    expect { broadcaster.announce(ThingHappened.new(id: "1")) }.not_to raise_error
  end
end
